#!/bin/bash
# Submit with: sbatch slurm_train_ssv2_our_mira_cc.sh
# Training: LAM on SSv2_our + MiraData (parquet) | CC cluster | 4x H100

#SBATCH --job-name=lam_ssv2_mira_cc_v2_equal_data
#SBATCH --output=/scratch/a/ankur99/WORLD_MODEL_PROJECT/logs/lam_ssv2_our_mira_cc_v2_equal_data_%j.out
#SBATCH --error=/scratch/a/ankur99/WORLD_MODEL_PROJECT/logs/lam_ssv2_our_mira_cc_v2_equal_data_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
CKPT_DIR="$SCRATCH/lam/lam_logs/exp_ckpts_ssv2_our_mira_v2_equal_data"
...
torchrun \
    ...
    main_cc.py fit \
    --config config/lam_ssv2_our_mira_cc.yaml \
    $RESUME_ARG \
    2>&1 | tee -a $SCRATCH/lam/lam_logs/output_train_ssv2_our_mira_cc_v2_equal_data.log
