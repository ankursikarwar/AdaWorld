#!/bin/bash
# Submit with: sbatch slurm_train_ssv2_our_mira_cc_seg.sh
# Training: LAM segment-level (T=8, causal temporal) on SSv2_our + MiraData | CC cluster | 4x H100

#SBATCH --job-name=lam_ssv2_mira_cc_seg
#SBATCH --output=/scratch/a/ankur99/WORLD_MODEL_PROJECT/logs/lam_ssv2_our_mira_cc_seg_%j.out
#SBATCH --error=/scratch/a/ankur99/WORLD_MODEL_PROJECT/logs/lam_ssv2_our_mira_cc_seg_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
CKPT_DIR="$SCRATCH/lam/lam_logs/exp_ckpts_ssv2_our_mira_seg_v3"
...
torchrun \
    ...
    main_cc.py fit \
    --config config/lam_ssv2_our_mira_cc_seg.yaml \
    $RESUME_ARG \
    2>&1 | tee -a $SCRATCH/lam/lam_logs/output_train_ssv2_our_mira_cc_seg.log
