#!/bin/bash
# Submit with: sbatch slurm_train_ssv2_clip_regression.sh
# Training: LAM + CLIP alignment (regression loss) on SSv2_our | Partition: long | Time: 24h

#SBATCH --job-name=lam_clip_reg
#SBATCH --output=/network/scratch/a/ankur.sikarwar/WORLD_MODEL_PROJECT/logs/lam_ssv2_clip_regression_%j.out
#SBATCH --error=/network/scratch/a/ankur.sikarwar/WORLD_MODEL_PROJECT/logs/lam_ssv2_clip_regression_%j.err
...
mkdir -p $SCRATCH/exp_ckpts_ssv2_clip_regression_v1
...
CKPT_DIR="$SCRATCH/exp_ckpts_ssv2_clip_regression_v1"
...
torchrun \
    ...
    --config config/lam_ssv2_clip_regression.yaml \
    $RESUME_ARG \
    2>&1 | tee -a $SCRATCH/output_train_ssv2_clip_regression.log
